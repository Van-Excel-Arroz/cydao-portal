...
export default function LoginPage() {
	...
	const { token, isStaff, setAuth } = useAuthStore();
	...
	if (token) {
		return <Navigate to={isStaff() ? '/admin/programs' : '/youth/profile'} replace />;
	}
	...
	async function onSubmit(data: LoginFormData) {
		...
		try {
			...
			setAuth(res.data);
			navigate(res.data.role === 'Staff' ? '/admin/programs' : '/youth/profile');
		} catch (err: unknown) {
			const message =
